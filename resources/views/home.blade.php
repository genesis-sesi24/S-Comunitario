            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-slate-50">
                    @forelse($patologias as $patologia)
                        <div class="p-4 flex items-center hover:bg-slate-50 transition-colors group">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center mr-4 
                                @if(Str::contains(strtolower($patologia->nombre), ['hipertensión', 'cardio'])) bg-red-50 text-red-600
                                @elseif(Str::contains(strtolower($patologia->nombre), 'diabetes')) bg-purple-50 text-purple-600
                                @elseif(Str::contains(strtolower($patologia->nombre), ['asma', 'epoc', 'respirat'])) bg-sky-50 text-sky-600
                                @else bg-slate-100 text-slate-500 @endif">
                                
                                @if(Str::contains(strtolower($patologia->nombre), ['hipertensión', 'cardio']))
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                                @elseif(Str::contains(strtolower($patologia->nombre), 'diabetes'))
                            ...
                            <div class="flex-1">
                                <h4 class="font-semibold text-slate-800">{{ $patologia->nombre }}</h4>
                                @if($patologia->tipo)
                                    <span class="text-xs text-slate-500">{{ $patologia->tipo }}</span>
                                @endif
                            </div>
                            
                            <span class="text-xs font-medium px-2 py-1 rounded bg-slate-100 text-slate-500">
                                Monitorizable
                            </span>
                        </div>
                    @empty
                        <div class="p-8 text-center text-slate-500">
                            No hay patologías registradas en el sistema.
                        </div>
                    @endforelse
